...
import { Plus, X, DollarSign, CheckCircle, Settings, Edit2, Save, Search, Trash2, Printer } from 'lucide-react';
...
const Hacienda = () => {
  ...
  const generarPDF = () => {
    ...
    // Configuración inicial de colores y fuentes
    const primaryColor = [37, 99, 235]; // Azul similar a la app
    const textColor = [60, 60, 60];
    
    // Título principal
    doc.setFontSize(22);
    doc.setTextColor(primaryColor[0], primaryColor[1], primaryColor[2]);
    doc.text('Reporte de Hacienda y Finanzas', 14, 20);
    ...
    // Resumen financiero
    const totalDeuda = obligaciones.filter(o => o.estado === 'Pendiente').reduce((acc, curr) => acc + Number(curr.monto_total), 0).toFixed(2);
    const totalRecaudado = obligaciones.filter(o => o.estado === 'Pagado').reduce((acc, curr) => acc + Number(curr.monto_pagado || curr.monto_total), 0).toFixed(2);

    doc.setFontSize(12);
    doc.setTextColor(textColor[0], textColor[1], textColor[2]);
    doc.text(`Deuda Total Pendiente: Bs ${totalDeuda}`, 14, 42);
    
    doc.setTextColor(16, 185, 129); // Verde success
    doc.text(`Total Recaudado: Bs ${totalRecaudado}`, 14, 50);

    // Tabla de obligaciones
    const tableColumn = ["Afiliado", "N°", "Tipo", "Concepto", "Monto (Bs)", "Venc.", "Estado"];
    const tableRows = [];

    filteredObligaciones.forEach(o => {
      const afiliadoNombre = `${o.afiliados?.perfiles?.nombres} ${o.afiliados?.perfiles?.paterno || ''}`.trim();
      const numeroAfiliado = o.afiliados?.numero_afiliado || 'S/N';
      const tipoData = o.tipo_obligacion;
      const conceptoData = o.concepto;
      const montoData = o.monto_total;
      const fechaData = new Date(o.fecha_limite).toLocaleDateString();
      const estadoData = o.estado;

      tableRows.push([afiliadoNombre, numeroAfiliado, tipoData, conceptoData, montoData, fechaData, estadoData]);
    });
